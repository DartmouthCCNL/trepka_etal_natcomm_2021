function [negloglike,lls, pl, pr]=funChoiceKernel(xpar,dat)
% % funDQ_RPE %
%PURPOSE:   Function for maximum likelihood estimation, called by fit_fun().
%
%INPUT ARGUMENTS
%   xpar:       alpha, beta, alpha2
%   dat:        data
%               dat(:,1) = choice vector
%               dat(:,2) = reward vector
%
%OUTPUT ARGUMENTS
%   negloglike:      the negative log-likelihood to be minimized

%%
alpha=xpar(1);
beta=xpar(2);
alpha2=xpar(3);
gamma = xpar(4);
omega = xpar(5);

nt=size(dat,1);
negloglike=0;

q_right=0.5;
q_left=0.5;
v_right=0.5;
v_left=0.5;
c_right=0.5;
c_left=0.5;

lls = zeros(1,nt);

pl = zeros(1,nt);
pr = zeros(1,nt);

for k=1:nt
    
    pright=exp(beta*v_right)/(exp(beta*v_right)+exp(beta*v_left));
    pleft=1-pright;
    
    if pright==0
        pright=realmin;   % Smallest positive normalized floating point number, because otherwise log(zero) is -Inf
    end
    if pleft==0
        pleft=realmin;
    end
    
    %compare with actual choice to calculate log-likelihood
    if dat(k,1)==1
        logp=log(pright);
    elseif dat(k,1)==-1
        logp=log(pleft);
    else
        logp=0;
    end
    lls(k) = -logp;
    negloglike=negloglike-logp;  % calculate log likelihood
    
    % update value for the performed action
    if dat(k,1)==1      %chose right
        if dat(k,2)>0
            q_right=q_right+alpha*(dat(k,2)-q_right);
        else
            q_right=q_right+alpha2*(dat(k,2)-q_right);
        end
        c_right = (1-gamma)*c_right+gamma;
        c_left = (1-gamma)*c_left;
    elseif dat(k,1)==-1 %chose left
        if dat(k,2)>0
            q_left=q_left+alpha*(dat(k,2)-q_left);
        else
            q_left=q_left+alpha2*(dat(k,2)-q_left);
        end
        c_left = (1-gamma)*c_left+gamma;
        c_right = (1-gamma)*c_right;
    end
    v_right = q_right + c_right*omega;
    v_left = q_left + c_left*omega;
end

end