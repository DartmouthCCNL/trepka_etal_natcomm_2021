...
%           string ('k'), or a truecolor vector ([1 0 0]). Use a
...
%         ah = subplot(3,4,12);
%         subplot(3,4,12),distributionPlot(chi2rnd(3,1000,1),'histOri','right','color','k','widthDiv',[2 2],'showMM',0)
...
for iData = goodData'
    ...
    if floor(opt.histOpt) == 1,
        colorOpt = [colorOpt {'EdgeColor', opt.color{iData}}];
    else
        colorOpt = [colorOpt {'EdgeColor', opt.color{iData}}];
    end
    ...
    if opt.showMM > 0
        ...
        if opt.showMM == 6
            ...
            md(iData,2,1) = prctile(currentData,2.5);
            md(iData,3,1) = prctile(currentData,97.5);
